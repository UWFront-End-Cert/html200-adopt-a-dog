...
//    console log form input details using jquery
    const formdata = $('input,select');

    console.log(formdata)

    for (let i = 0; i < formdata.length - 1; i++) {

        const id = $('input,select').elements[i].id;
        const value = $('input,select').elements[i].value;
        const checked = $('input,select ').elements[i].checked;
        const submitdata = (id + ": " + value);;
        const selection = ("first time adopter?: " + value)
